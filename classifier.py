...
def classify(args, train_X, train_y, dev_X, dev_y, test_X, test_y, feat_dim, num_classes):
  ...
  props, scores = [], []
  for reg in regs:
    clf = MLP(classifier_config, inputdim=feat_dim, nclasses=num_classes, l2reg=reg, seed=args.seed)
  ...
  clf = MLP(classifier_config, inputdim=feat_dim, nclasses=num_classes, l2reg=opt_prop[0], seed=args.seed)
  clf.fit(train_X, train_y, validation_data=(dev_X, dev_y))
